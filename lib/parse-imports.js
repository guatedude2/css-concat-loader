  ...
  function findImports(filename, source) {
    ...
    while(match) {
      ...
      if (match) {
        ...
        var isOptional = /optional/.test(match[1]);
        var isCss = /css/.test(match[1]);
        var found = false;
        var fileLookup = [];
        ...
        // if the import was not found and wasn't marked as optional or css then error out
        if (!found && !isOptional && !isCss) {
