...
module.exports = function (filename, content, config) {
  ...
  function findImports(filename, source) {
    ...
    while(match) {
      match = constants.IMPORT_REGEX().exec(source);
      ...
      if (match) {
        ...
        for (var i = 0; i < fileLookup.length && !found; i++) {
          ...
          if (content !== null) {
            ...
            constants.IMPORT_REGEX().lastIndex = match.index + output.length;
            found = true;
          }
        }
    ...
    if (config.resolveUrls) {
      return source.replace(constants.URL_REGEX(), function (m, q, url) {
